      ...
      if (std::abs(trackGen.eta()) < BDT_track_eta_cut && trackGen.pt() > BDT_track_pt_cut) {
        ...
        if (found != r2s_->end()) {
          ...
          if (withMTD) {  // TP link to MTDsimCluster
            ...
            for (const auto& SimCluRefs : simClustersRefs->val){
                                            ...
                                            for (unsigned int i = 0; i < simClustersRefs_RecoMatchBTL.size(); i++) {
                                                ...
                                                if ((*simClusterRef_RecoMatchBTL).simLCTime() ==
                                                    ...
                                                    if ((*SimCluRefs).trackIdOffset() == 0) { // SimCluster linked to TP is from DirectHit!!!
                                                        direct_type = true;
                                                        other_type = false;
                                                    }else{ // SimCluster linked to TP is from Other type of Hit!!!
                                                        direct_type = false;
                                                        other_type = true;
                                                    }
            ...
            // We are saving cluster type correctly for good associactions and cases where there is just 1 SimClu for TP.
            // If we have 2 SimClu and both of them do not pass the good association criteria, then which SimClu should be saved? First/second etc.? All? 
            // If we save all, then we would need multiple entries for each track to save all the possible clusters and their types etc.
            // For now, if we have multiple SimClu that fail associaiton criteria, we are saving the type of the first cluster.
            if(good_association == false){
                const auto& SimCluRefss = (simClustersRefs->val)[0];
                if ((*SimCluRefss).trackIdOffset() == 0) { // SimCluster linked to TP is from DirectHit!!!
                    direct_type = true;
                    other_type = false;
                }else{ // SimCluster linked to TP is from Other type of Hit!!!
                    direct_type = false;
          ...
          Ttrack_DirClu.push_back(direct_type);
          Ttrack_OtherClu.push_back(other_type);
