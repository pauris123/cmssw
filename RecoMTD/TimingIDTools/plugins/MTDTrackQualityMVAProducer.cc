...
private:
  ...
  edm::EDGetTokenT<reco::BeamSpot> RecBeamSpotToken_; // Added for updated BDT
  ...
  edm::EDGetTokenT<edm::ValueMap<float>> sigmamtdTimeToken_; // Added for updated BDT
  edm::EDGetTokenT<edm::ValueMap<float>> pathLengthToken_;
  edm::EDGetTokenT<edm::ValueMap<int>> npixBarrelToken_;
  edm::EDGetTokenT<edm::ValueMap<int>> npixEndcapToken_;
  edm::EDGetTokenT<edm::ValueMap<float>> outermostHitPositionToken_; // Added for updated BDT
...
MTDTrackQualityMVAProducer::MTDTrackQualityMVAProducer(const ParameterSet& iConfig)
    : tracksToken_(consumes<reco::TrackCollection>(iConfig.getParameter<edm::InputTag>("tracksSrc"))),
      RecBeamSpotToken_(consumes<reco::BeamSpot>(iConfig.getParameter<edm::InputTag>("offlineBS"))), // Added for updated BDT
      ...
      sigmamtdTimeToken_(consumes<edm::ValueMap<float>>(iConfig.getParameter<edm::InputTag>("sigmamtdTimeSrc"))), // Added for updated BDT
      ...
      outermostHitPositionToken_(consumes<edm::ValueMap<float>>(iConfig.getParameter<edm::InputTag>("outermostHitPositionSrc"))), // Added for updated BDT
...
void MTDTrackQualityMVAProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("sigmamtdTimeSrc", edm::InputTag("trackExtenderWithMTD", "generalTracksigmatmtd"))
      ->setComment("sigma MTD Time value Map"); // Added for updated BDT
  ...
  desc.add<edm::InputTag>("outermostHitPositionSrc", edm::InputTag("trackExtenderWithMTD", "generalTrackOutermostHitPosition")); // Added for updated BDT
  desc.add<edm::InputTag>("offlineBS", edm::InputTag("offlineBeamSpot")); // Added for updated BDT
  /*desc.add<edm::FileInPath>("qualityBDT_weights_file",
                            edm::FileInPath("/afs/cern.ch/user/n/nstrautn/CMSSW_14_2_0_pre1/src/RecoMTD/TimingIDTools/test/4DMTD_tree_v2_wDispl_d7_Reg.xml"))
      ->setComment("Track MTD quality BDT weights");*/
  desc.add<edm::FileInPath>("qualityBDT_weights_file",
                            edm::FileInPath("RecoMTD/TimingIDTools/data/4DMTD_tree_v2_wDispl_d7_Reg.xml"))
      ->setComment("Track MTD quality BDT weights");
