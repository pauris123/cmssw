...
private:
  ...
  //std::vector<unsigned int> Ttrack_idOffset_sub;
  //std::vector<std::vector<unsigned int>> Ttrack_idOffset;
      ...
      if(std::abs(trackGen.eta()) < BDT_track_eta_cut && trackGen.pt() > BDT_track_pt_cut && track_vtx_dz_check < BDT_track_dz_cut){
        ...
        if (found != r2s_->end()){
          ...
          if (withMTD){ // TP link to MTDsimCluster
            ...
            //   nGoodCluster++; 
            //   //Ttrack_idOffset_sub.push_Back((*simClusterRef).trackIdOffset());
            //   if((*SimCluRefs).trackIdOffset() == 0){
            //     Ttrack_TPSimCluDirect.push_back(true);
            //     Ttrack_nSimClust.push_back(nGoodCluster);
            //     //directHit = true; 
            //     break; // if cluster with direct hit found, save that it was a direct hit cluster and its number in the set of clusters
            //   }
            // }
